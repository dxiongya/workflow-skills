...
每条验收标准**必须对应一个可执行的验证动作**（命令 / 文件路径 / grep / 编译 / 测试），结卡时**必须贴出该动作的实际输出**。

模糊描述如"已验证"、"OK"、"差不多了"、"看起来没问题"**不接受** —— 这是防 LLM 幻觉的核心纪律：没有输出 = 没有验证。

### 推荐语法（格式约定，非强制）

为便于扫读与复核，推荐用下列简写形式书写验证动作。但验收标准本身可以用自然语言，只要结卡时能贴出对应的命令与输出即可。
...
混合使用时一行一条。写卡人也可以用散文描述验收标准（如"components.css 存在且至少 13 条语义类"），只要结卡时贴出等价的命令输出。
