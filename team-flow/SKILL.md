...
|----|------|
| **卡片目录** | 项目根下 `tasks/` |
| **索引文件** | `tasks/README.md`（模板：`templates/tasks-readme-template.md`） |
| **卡片文件** | `tasks/<PREFIX>-<NN>.md`，`PREFIX` ∈ {`REFACTOR` / `FEAT` / `BUG` / `CHORE`}，`NN` 两位零填充 |
| **新卡模板** | `templates/card-template.md`（直接 cp 过来改） |
| **卡片 ↔ commit** | commit message 首行必须引用 `CARD-ID`，详见 `references/dev.md` |
